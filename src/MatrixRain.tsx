...
let width,height,columns,yPositions=[];const fontSize=16,columnWidth=14;
...
function step(){ctx.fillStyle='rgba(0, 0, 0, 0.12)';ctx.fillRect(0,0,width,height);ctx.font=fontSize+'px Consolas, Menlo, monospace';ctx.shadowColor='#00ffb3';ctx.shadowBlur=8;for(let col=0;col<columns;col++){const ch=chars[Math.floor(Math.random()*chars.length)],x=col*columnWidth,y=yPositions[col]*(fontSize+2);ctx.fillStyle='#00ffb3';ctx.fillText(ch,x,y);ctx.fillStyle='#baffea';ctx.fillText(ch,x,y-fontSize*.9);if(y>height&&Math.random()>.975)yPositions[col]=0;else yPositions[col]++}ctx.shadowBlur=0;ctx.fillStyle='rgba(0,255,179,0.05)';for(let s=0;s<8;s++)ctx.fillRect(Math.random()*width,Math.random()*height,2,2);requestAnimationFrame(step)}
